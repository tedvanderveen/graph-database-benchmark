...
CLIENTS_PER_WORKER=${CLIENTS_PER_WORKER:-$(grep -c ^processor /proc/cpuinfo 2>/dev/null || echo 8)}
...
python3 kn.py --url ${DATABASE_HOST}:${DATABASE_PORT} -g graph500_22 -s ${DATA_NODE_FILE_NAME} \
    ...
    --threads ${WORKERS} --clients ${CLIENTS_PER_WORKER} -i ${ITERATIONS_PER_QUERY} --stdout || exit 1
